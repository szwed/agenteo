...
Crossplane is a CNCF Graduated project that turns any Kubernetes cluster into a cloud infrastructure control plane. It lets platform teams build custom APIs (using the [KRM](KRM.md) pattern) without writing controllers, and continuously reconciles desired state to actual state across cloud providers.
...
- **Composite Resource Definitions (XRDs)** -- Higher-order CRDs that define platform-specific APIs (see [Digital Twins Extensibility](Digital-Twins-Extensibility.md)). An XRD creates a new API endpoint (e.g. `mydatabases.example.org`) with an OpenAPI v3 schema. Developers consume the simplified API without knowing the underlying resources.
...
Crossplane is the canonical example of the **Actuator** archetype (see [Controller Archetypes](Controller-Archetypes.md)): Provider controllers map real-world cloud capabilities into the Kubernetes virtual world as digital twins. It extends [KRM](KRM.md) beyond the cluster boundary to cloud resources, making infrastructure management indistinguishable from application management at the API level.

In the [Infrastructure as Code](Infrastructure-as-Code.md) taxonomy, Crossplane is the primary **continuously reconciled** IaC tool -- drift is corrected automatically by the controller loop, unlike occasionally reconciled tools (Terraform, Pulumi) that only detect drift on the next run.

## GitOps Integration

Crossplane integrates with ArgoCD (see [CI CD](CI-CD.md)) for GitOps-driven infrastructure. Configuration requirements:
...
- [Infrastructure as Code](Infrastructure-as-Code.md) -- Crossplane as continuously reconciled IaC
- [KRM](KRM.md) -- Kubernetes Resource Model that Crossplane extends to cloud resources
- [Controller Archetypes](Controller-Archetypes.md) -- Crossplane Providers as actuator archetype
- [Digital Twins Extensibility](Digital-Twins-Extensibility.md) -- XRDs as higher-order CRDs
- [CI CD](CI-CD.md) -- ArgoCD integration for GitOps
- [Controller Pattern](Controller-Pattern.md) -- Reconciliation loop underpinning Crossplane
